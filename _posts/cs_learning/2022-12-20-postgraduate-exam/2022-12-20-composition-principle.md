...
- 硬件的基本组成
  ...
  - 控制器：
    - 程序计数器 PC：存放当前欲执行指令的地址，可以自动+1 形成下一条指令地址，与 MAR 有直接通路
    - 指令寄存器 IR：存放当前的指令，来自 MDR
    - 控制单元 CU
...
- 性能指标：速度越高，价格越高；容量越大，价格越低，速度也越低。理想大容量、高速度、低价格
  ...
  - 单位成本：每位价格=总成本/总容量
  - 存储速度：数据传输率=数据的宽度/存储周期。存储周期又称读写周期或访问周期，指连续两次独立地访问存储器操作之间所需的最小时间间隔；存取时间指启动一次存储器操作到完成该操作所经历的时间，一般小于存取周期（需要复原时间）
...
![截屏2020-10-11 下午8.41.30](/Users/P1anet/Desktop/截屏 2020-10-11 下午 8.41.30.png)
...
- SRAM
  ...
  - 保持存储信息：使用六管静态 MOS 存储单元电路，利用双稳态触发器保存信息
  - 读数据：送地址且发读命令
  - 写数据：需要传输地址信号和数据信号，$\overline{W}$读 0 写 0 高电平，$W$读 1 写 1 低电平

- DRAM

  - 保持存储信息：分多管型和单管型，遵循电容存储电荷的原理
  ...
  - ![截屏2020-10-11 下午9.25.21](/Users/P1anet/Desktop/截屏 2020-10-11 下午 9.25.21.png)
...
- 比较
  ...
  - DRAM：破坏性读出，**需要刷新**。断电信息即丢失，易失性存储器。**集成度高，位价低，容量大，功耗低**。存取速度比 SRAM 慢，常用于大容量的主存系统
...
- 近期最少使用算法（LRU）：替换出近期用得最少的字块，记录**每个块**最近一次使用的时间。平均命中率高（堆栈类算法）
- 最不经常使用法 LFU
- 随机法 RAND
...
- 硬盘存储器：记录介质为硬质圆形盘片的磁表面存储设备
  ...
  - 按是否具有可换性分为可换盘磁盘存储器和固定盘磁盘存储器
  - 按工作方式分为固定磁头磁盘存储器和移动磁头磁盘存储器
...
- 总线的传输周期：CPU 通过总线对存储器或 I/O 端口进行一次访问所需的时间。申请，寻址，传输，结束
- 总线宽度。总线特性：机械(尺寸、形状)；电气(传输方向、有效电平范围)；功能(每根传输线的功能)；时间(信号和时序关系)
- 猝发传输：一个总线周期内传输存储地址连续的多个数据字
- 按数据传送方式分为并行、串行传输总线，并行又可按位数分为 8、16、32、64 位等
- 按使用范围分为计算机总线、测控总线等
- 按连接部件不同分为
...
- 性能指标
  - 总线时钟周期即机器的时钟周期，总线时钟频率
  - 总线传输周期指一次总线操作所需的时间，包含若干总线时钟周期，总线工作频率
  - 总线宽度：指**数据总线**的根数
  - 总线带宽：单位时间内总线上传输数据的位数=总线宽度 x 总线工作频率
  - 总线复用：地址总线和数据总线共用一组线，分时
  - 信号线数：地址总线、数据总线和控制总线 3 种总线数的总和
- 总线的结构
  - 单总线结构：结构简单，容易扩充。影响工作效率，不能解决 CPU、主存、I/O 设备之间传输速率不匹配的问题。多被小型计算机或微型计算机采用。**主存和 I/O 设备统一编址**
  - 双总线结构：将**I/O 总线**与**主存总线**分开
  - 三总线结构：增加一条**DMA 总线**用于高速外设和主存之间直接交换信息。任意时刻只能使用一种总线：主存总线和 DMA 总线不能同时对主存进行存取，I/O 总线只有在 CPU 执行 I/O 指令时才用到
- 总线仲裁，又称总线判优控制
  - 主设备对总线有控制权，从设备只能响应从主设备发来的总线命令，对总线没有控制权
  - 一根总线可以连接在若干设备上，有一个或多个主控设备，在一个总线传输周期内，一根总线只能有一个主控设备控制总线，选择一个从设备与之进行通信
  - 集中仲裁，集中控制优先权仲裁方式
    ...
    - **确定总线使用权(控制线总数要再+1 表示总线忙)：链式 2 根，计数器 log~2~n+1，独立请求 2n 根**
  - 分布仲裁：不需要中央仲裁器
    - 每个主模块有自己的仲裁号和仲裁器，通过发送仲裁号到共享的仲裁总线上进行竞争，优先级低的不响应且撤销仲裁号，获胜者的仲裁号保留在仲裁总线上
- 总线周期：完成一次总线操作的时间。分为申请分配阶段，寻址阶段，传送数据阶段，结束阶段
- 总线通信控制：主要解决通信双方如何获知传输开始和传输结束，以及通信双方如何协调和配合
  - 同步通信，同步定时方式：统一的时钟信号协调发送和接收双方的传送定时关系
    ...
    - 优点：传送速度快，具有较高的传输速率
    - 缺点：必须按最慢的模块来设计公共时钟，损失总线效率；不知道外设是否真正响应，可靠性较低
